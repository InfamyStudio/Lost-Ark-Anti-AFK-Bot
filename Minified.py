...
def LAForeground():A=win32com.client.Dispatch('WScript.Shell');A.SendKeys('%');win32gui.SetForegroundWindow(hwnd)
def LALocation():
	if LAWindowTitle:
		if hwnd:A=win32gui.GetWindowRect(hwnd);B=A[0];C=A[1];D=A[2]-B;E=A[3]-C;return B,C,D,E
		else:print('Window not found!')
def MouseClick(retScreenSize):LAForeground();D=retScreenSize;A=RandomLocationGen(D);B=A[0];C=A[1];pyautogui.moveTo(B,C);pyautogui.click();print(_C+str(B)+','+str(C)+')')
def ButtonSetup():
